...
{
	public static readonly Color SerializedReferenceMenuBackgroundColor = Color.white; //new Color(0.1f, 0.55f, 0.9f, 1f);
		 ...
		 Rect position, Color color, IEnumerable<Func<Type, bool>> filters = null)
	{
		...
		var className = string.IsNullOrEmpty(names.ClassName) ? "Null (Assign)" : property.managedReferenceFullTypename.Split(new char[] { '.' }).Last();
